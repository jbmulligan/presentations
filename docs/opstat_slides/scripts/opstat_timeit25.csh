...
# We give warnings at 10 minutes, 15 minutes, 18 minutes, 19 minutes, and 20
...
while( 1 )
  ...
  if( $state == 1 ) then
    ...
    show_alert "$total_delay minutes elapsed, should be on 2nd task..."
    @ next_time += $second_delay * $seconds_per_minute
    @ state ++
  else if( $state == 2 ) then
    @ total_delay += $second_delay
    show_alert "$total_delay minutes elapsed, should be on 3rd task..."
